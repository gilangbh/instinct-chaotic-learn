      ...
      <div className="col-span-12 flex justify-between items-end border-b border-zinc-800/50 pb-6 relative">
        ...
        <div className="flex gap-8 text-right">
          <div className="group">
            ...
            <div className="text-3xl font-mono text-white">$2,450.00 <span className="text-emerald-500 text-sm">(+12%)</span></div>
          </div>
          <div className="group">
            ...
            <div className="text-3xl font-mono text-indigo-400">#842</div>
          </div>
        </div>
      </div>
      ...
      <div className="col-span-12 lg:col-span-8 space-y-6">
        ...
        <Panel active className="p-8 cursor-pointer relative overflow-hidden group h-full flex flex-col justify-between min-h-[400px]" onClick={onEnterRun}>
          <div className="absolute top-0 right-0 p-2 px-4 text-xs font-mono text-indigo-300 border-b border-l border-indigo-500/30 bg-indigo-500/10 backdrop-blur-md z-20">
            <span className="animate-pulse">●</span> STATUS: {activeRun.status.toUpperCase()} [{activeRun.currentRound}/{activeRun.totalRounds}]
          </div>
          ...
          <div className="relative z-10">
            <div className="flex items-center gap-6 mb-8">
              ...
              <div>
                <h2 className="text-4xl text-white font-light mb-2 tracking-tight">{activeRun.coin}-PERP <span className="text-zinc-600 text-2xl">/</span> USDC</h2>
                <div className="flex items-center gap-3 text-sm font-mono">
                  <Badge label="VOLATILITY: HIGH" color="cyan" pulse />
                  <span className="text-zinc-700">|</span>
                  <span className="text-emerald-400 flex items-center gap-1"><TrendingUp size={14}/> YIELD: +24%</span>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-4 mb-10">
              {[
                { label: "Pool Liquidity", val: `$${formatUSDC(activeRun.totalPool)}`, icon: Box, color: "text-white" },
                { label: "Active Nodes", val: activeRun.participantCount.toString(), icon: Cpu, color: "text-indigo-300" },
                { label: "Consensus", val: "83%", icon: Radar, color: "text-emerald-300" }
              ].map((stat, i) => (
               ...
               {runHistory.slice(0, 5).map((run, i) => (
                  ...
                  >
                     <div className="flex items-center gap-3">
                        <div className={`w-1.5 h-1.5 rounded-full shadow-[0_0_8px_currentColor] ${run.totalPool >= run.startingPool ? 'bg-emerald-500 text-emerald-500' : 'bg-red-500 text-red-500'}`} />
                        <span className="font-mono text-zinc-400 group-hover:text-white transition-colors">#{run.id} {run.tradingPair}</span>
                     </div>
                     <div className="flex items-center gap-2">
                        <div className={`font-mono ${run.totalPool >= run.startingPool ? 'text-emerald-500' : 'text-red-500'} font-bold`}>
